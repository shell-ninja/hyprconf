...
def build_dotfiles_update_script() -> str:
    ...
    Returns a bash script (as a string) that:
      ...
      4. Prompts the user to reboot the system (required), with an animated
         countdown bar and reboot command following ~/test-reboot.sh.
...
class HyprSettingsWindow(Adw.ApplicationWindow):
    ...
    def _build_dotfiles_update_page(self):
        ...
        action_row = Adw.ActionRow(
            title="Update now",
            subtitle="Fetches the branch via curl, extracts it, runs "
            "setup.sh in a new kitty window, and prompts to reboot.",
        )
